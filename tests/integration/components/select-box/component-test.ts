...
module('Integration | Component | select-box', function (hooks) {
  ...
  test('selectbox  active on click (type=input box)', async function (assert) {
    ...
    this.set('selected', 'hi1');
    await render(hbs`{{select-box  data=this.data selected=this.selected  }}`);
    await click('input');
  ...
  test(' check isDefaultOpen true', async function (assert) {
    ...
    this.set('selected', 'hi1');
    await render(hbs`{{select-box  data=data selected=selected isDefaultOpen=true}}`);
    assert.equal(document.querySelectorAll('.popper').length, 1)
  });
  ...
  test(' check class arg rendering', async function (assert) {
    ...
    this.set('selected', 'hi1');
    await render(hbs`{{select-box  data=data selected=selected  class='select-rendering' }}`);
  ...
  test(' placeholder is rendering ', async function (assert) {
    ...
    this.set('data', data);
    this.set('selected', 'hi2');
    await render(hbs`{{select-box  data=data selected=selected placeholder='any text'  }}`);
  ...
  test(' active=true is rendering ', async function (assert) {
    ...
    this.set('data', data);
    this.set('selected', 'hi2');
    await render(hbs`{{select-box type='button' data=data selected=selected active=true placeholder='any text'  }}`);
  ...
  test(' minimal=true is rendering ', async function (assert) {
    ...
    this.set('data', data);
    this.set('selected', 'hi2');
    await render(hbs`{{select-box type='button' data=data selected=selected minimal=true placeholder='any text'  }}`);
